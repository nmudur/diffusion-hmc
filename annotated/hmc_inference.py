...
class TimeBatchedCondModel(nn.Module):
        ...
        def get_log_probability(self, param, eval_gradients=True):
            ...
            if ((param[0]-self.PMIN[0])>=0) and ((param[1]-self.PMIN[1])>=0) and ((self.PMAX[0] - param[0])>=0) and ((self.PMAX[1] - param[1])>=0):
                with torch.set_grad_enabled(True): # This is the default, but let's be explicit.
                    ...
                    for it, tbatch in enumerate(self.timestep_batches):
                        ...
                        if it==0:
                            ...
                            vlb_loss = -(nll.sum()+kl_divs.sum())  # sum over timesteps. This is actually the negative of the vlb_loss.
                ...
                vlb_loss = torch.tensor(-1e15, requires_grad=True) # Let's use 1e8 as a standin for inf
                if self.compute_gradients and eval_gradients:
                    # find which prior boundary was violated
                    GRADFAC = 100 # gradient when outside prior = -10.0\Delta p
                    if (param[0] - self.PMIN[0]) < 0:
                        accumulated_grad = torch.tensor([-GRADFAC*(param[0] - self.PMIN[0]), 0.0]).to(torch.float32).to(param.device)
                    elif (param[1] - self.PMIN[1]) < 0:
                        accumulated_grad = torch.tensor([0.0, -GRADFAC*(param[1] - self.PMIN[1])]).to(torch.float32).to(param.device)
                    elif (self.PMAX[0] - param[0]) < 0:
                        accumulated_grad = torch.tensor([-GRADFAC*(param[0] - self.PMAX[0]), 0.0]).to(torch.float32).to(param.device)
                    elif (self.PMAX[1] - param[1]) < 0:
                        accumulated_grad = torch.tensor([0.0, -GRADFAC*(param[1] - self.PMAX[1])]).to(torch.float32).to(param.device)
                    else:
                        raise ValueError("No prior boundary violated, but still outside prior")

                    print('Outside prior, gradient=', accumulated_grad)
            if self.compute_gradients and eval_gradients:
...
outdir = f"{exp_dir}/{noisename}_field-{id}/"
#outdir = f'results/hmc_samps/exp2_Ntime/{date_format}/'
